...
<?php while ( have_posts() ) : the_post(); ?>
    ...
    <?php get_template_part( 'template-parts/sections/section-single_article' ); ?>
...
<?php if ( have_rows( 'blog_default_sections' ) ) : ?>
    <?php while ( have_rows('blog_default_sections' ) ) : the_row();
        if ( get_row_layout() == 'quick_order' ) :
            get_template_part('template-parts/blocks-blogs/section', 'quick_order');

        elseif ( get_row_layout() == 'blog_related_articles' ) :
